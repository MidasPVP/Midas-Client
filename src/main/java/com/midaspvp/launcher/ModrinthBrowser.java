...
public final class ModrinthBrowser {
	...
	public record ModResult(String slug, String title, String description, String iconUrl, int downloads) {
	}

	/** Blocking — run off the JavaFX thread. Returns up to 20 results, most-downloaded first. */
	public static java.util.List<ModResult> search(String query, String mcVersion) throws IOException, InterruptedException {
		String encodedQuery = URLEncoder.encode(query, StandardCharsets.UTF_8);
		String facets = URLEncoder.encode(
				"[[\"project_type:mod\"],[\"categories:fabric\"],[\"versions:" + mcVersion + "\"]]", StandardCharsets.UTF_8);
		String url = "https://api.modrinth.com/v2/search?query=" + encodedQuery
				+ "&facets=" + facets + "&limit=20&index=relevance";
		...
		java.util.List<ModResult> results = new java.util.ArrayList<>();
