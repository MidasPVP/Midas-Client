...
jpackage --type exe --name "Midas Client" --input jpackage-input \
  --main-jar MidasLauncher.jar --main-class com.midaspvp.launcher.Launcher \
  --app-content mods-bundle --dest dist-exe --vendor MidasPVP --app-version 1.3.0 \
...
`--type exe` produces a WiX Burn-bootstrapped `.exe` installer (what's actually shipped in releases);
swap it for `--type msi` to build a plain MSI instead — both come from the same jpackage input and
behave identically once installed. `--icon` needs a real `.ico` file; `src/main/resources/icons/logo.ico`
is generated by rendering the gold "M" wordmark at multiple sizes (see `IconBuild` history in git log
if you need to regenerate it after changing the logo).
